...
| **Lookup token** | — | **`Face_Skin`** (aliases resolve `Face.Skin` + source names) |
...
## Workflow underscore notation (Phase B standardize)

VRoid `_00_` is a **category separator**. Phase B converts tails to `{Region}_{Part}` with Title Case. Class suffixes (`-NoRim`) are separate.

| VRoid tail | Workflow |
|------------|----------|
| `Face_00_SKIN` | `Face_Skin` |
| `Body_00_SKIN` | `Body_Skin` |
| `FaceMouth_00_FACE` | `Mouth_Face` |
| `FaceBrow_00_FACE` | `Brow_Face` |
| `EyeIris_00_EYE` | `Iris_Eye` |
| `EyeHighlight_00_EYE` | `EyeHighlight_Eye` |
| `EyeWhite_00_EYE` | `EyeWhite_Eye` |
| `HairBack_00_HAIR` | `Hair_Back` |
| `Hair_00_HAIR` | `Hair_Back` |
| `Hair_00_HAIR_01` | `Hair_01` |
| `Shoes_01_CLOTH` | `Shoes_Cloth` |
| `Tops_01_CLOTH_01` | `Hoodie_01_Cloth` |
| `Tops_01_CLOTH_02` | `Hoodie_02_Cloth` |
| `Accessory_RabbitEar_01_CLOTH` | `RabbitEar_Cloth` |
| `Accessory_RabbitTail_01_CLOTH` | `RabbitTail_Cloth` |

Texture slugs = identity lowercased (`Face_Skin` → `face_skin`). Ignore `-Class` tail.
...
|-------------------------------|---------------|-------|
| `N00_005_01_Tops_01_CLOTH (Instance)` | `Hoodie_Cloth` | Single hoodie layer |
| `N00_005_01_Tops_01_CLOTH_01 (Instance)` | `Hoodie_01_Cloth` | Hoodie layer 1 |
| `N00_005_01_Tops_01_CLOTH_02 (Instance)` | `Hoodie_02_Cloth` | Hoodie layer 2 |
| `N00_001_02_*_CLOTH (Instance)` | `*_CLOTH` | Outfit slot `N00_001_02_` — uniform **vest** on many models |

**Layer rule:** stacked hoodie layers → `Hoodie_01_Cloth`, `Hoodie_02_Cloth`, …
...
mat = resolve_material_by_token("Face_Skin")  # workflow name; also finds Face.Skin / source if Phase B not run
...
2. `MToon Outline (Face_Skin-NoRim)` → `outline_face_skin`
3. Otherwise lowercase identity: `Body_Skin-NoRim` → `body_skin`
